...
import LoadingSection from "./LoadingScreen";
...
export default function TikTokCard() {
  ...
  const handleClick = () => {
    ...
    setTimeout(() => {
      router.push("/tiktok");
    }, 1000); // delay loading 1 detik, bisa diubah
  };
  ...
  return (
    <>
      ...
      >
        ...
        <AnimatePresence>
          {isHovered && (
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ duration: 0.4 }} className="absolute inset-0 bg-gradient-to-r from-pink-500 via-red-500 to-blue-500 z-0" aria-hidden="true" />
          )}
        </AnimatePresence>
