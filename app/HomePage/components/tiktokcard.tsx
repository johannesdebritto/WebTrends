...
export default function TikTokCard() {
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {isHovered && <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ duration: 0.4 }} className="absolute inset-0 bg-gradient-to-r from-pink-500 via-red-500 to-blue-500 z-0" />}
      </AnimatePresence>
      ...
      <div className="relative z-10 flex flex-col items-center justify-center h-full text-xl font-semibold text-black">
        {!isHovered && (
          <>
            <img src="/images/tiktok.svg" alt="TikTok Logo" className="w-12 h-12 mb-4 transition-opacity duration-200" />
            <p>Trends TikTok</p>
          </>
        )}
